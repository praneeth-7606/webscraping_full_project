...
//     // Function to summarize content from a product page
//     const summarizeContent = async (url) => {
//       try {
//         const response = await axios.get(url);
//         const $ = cheerio.load(response.data);
//         const content = $('body').text().trim();
//         return content.substring(0, 300) + '...'; // Summarize the content (first 300 chars)
//       } catch (error) {
//         console.error('Failed to summarize content:', error.message);
...
//           if (loc && image && title) {
//             const summary = await summarizeContent(loc);
...
//           // Extract meaningful content; adjust the selector based on the website's structure
//           const content = $('body').text().trim(); // Default to body text
      
//           // Split the content into paragraphs or sections
//           const paragraphs = content.split('\n').filter(p => p.trim().length > 0);
      
       
//           const bulletPoints = paragraphs.slice(0, 2).map((p, index) => `• ${p.trim()}`).join('\n');
      
//           return bulletPoints || 'No summary available';
//         } catch (error) {
//           console.error('Failed to summarize content:', error.message);
//           return 'No summary available';
//         }
...
//               const summary = await summarizeContent(loc); // Get the summary for each URL
...
// // GroqCloud API configuration
// const GROQ_API_KEY = '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';
// const GROQ_API_URL = 'https://api.groq.com/openai/v1/chat/completions';
...
//       messages: [
//         { role: "system", content: "You are a helpful assistant that summarizes web content." },
//         { role: "user", content: `Summarize the following content in about 3-4 paragraphs: ${content}` }
//       ],
//       max_tokens: 500
//     }, {
//       headers: {
...
// // Example usage
// const inputUrl = 'https://skindulge.in/products/3-in-1-skin-hydrator';
// summarizeUrl(inputUrl)
//   .then(summary => {
//     if (summary) {
